...
public class FieldrefInfo implements ConstantType {
	private byte[] index;
	private byte[] index2;
	...
	public void setIndex(byte[] index) {
		if (index == null || index.length != Constant.U2) {
			throw new RuntimeException("FieldrefInfo的index不能为空且长度必须为2字节");
		}
		this.index = index;
	}
	...
	public void setIndex2(byte[] index2) {
		if (index2 == null || index2.length != Constant.U2) {
			throw new RuntimeException("FieldrefInfo的index2不能为空且长度必须为2字节");
		}
		this.index2 = index2;
	}
	...
	@Override
	public int getTag() {
		return ConstantPoolTag.CONSTANT_Fieldref_info;
	}
